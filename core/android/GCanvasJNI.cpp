...
JNIEXPORT void JNICALL Java_com_taobao_gcanvas_GCanvasJNI_putImageData(
        JNIEnv *je, jclass jc, jstring contextId, jstring imageBase64, jint dx, jint dy, jint x,
        jint y, jint width, jint height) {
    LOG_D("Canvas JNI::putImageData dxy=(%d, %d), xy=(%d, %d), wh=(%d, %d)", dx, dy, x, y, width,
          height);
    GCanvasManager *theManager = GCanvasManager::GetManager();
    char *cid = jstringToString(je, contextId);
    string canvasId = cid;
    ...
    const char *imageStr = je->GetStringUTFChars(
            imageBase64, 0);
    int length = je->GetStringUTFLength(imageBase64);
    theCanvas->PutImageData(imageStr,length,dx,dy,x,y,width,height);
    je->ReleaseStringUTFChars(imageBase64, imageStr);
}
